...
| Option                       | Default | Meaning                                                                                                                                                                                  |
|------------------------------|---------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `feral_ordering`             | `auto`  | Fill-reducing ordering method (see table below). `auto` lets feral's adaptive dispatcher pick per-matrix; the concrete variants pin one method. `auto_race` races four symbolic passes and keeps the sparsest `L` — it is **not** a safe default (gh#768).            |
...
| `auto_race` | Race-based dispatcher: runs full symbolic factorization on AMD, MetisND, ScotchND, KahipND and keeps the smallest `factor_nnz`. ~4× a single symbolic pass, paid once per problem (symbolic factorization is cached across numeric refactorizations with the same pattern). **Measured a net loss on the Mittelmann NLP suite** (gh#768): 27 of 42 instances slower, 1 faster, median **0.82×** — the worst of the ten options screened, on a population where `LinearSystemFactorization` is 44–98 % of `OverallAlgorithm` on 36 of 47 instances. Two independent reasons, both structural: it often re-derives the ordering `auto` already picked (`ex8_2_2`: `last_nnz_l` 85662 either way, 0.32 s → 0.56 s), and **smallest `factor_nnz` is not the same objective as smallest wall clock** — on `robot_a` METIS and KaHIP carry ~1.9 % *more* nonzeros in `L` and factor 25–31 % faster, so the race keeps AMD and spends 4.06 s of symbolic (against AMD's 0.56 s) to select the loser. Even where ordering pays hugely it is beaten by a pin: `qssp180` 45.06 s default → 14.63 s `amd` (3.08×), 23.6 s `metis`, 28.7 s `auto_race` (1.57×). Treat it as a *diagnostic* — one run tells you how much fill the four methods disagree about — not as a setting to leave on. To pick an ordering, sweep the concrete variants and pin the winner. |
...
When in doubt: leave `feral_ordering` at the default. A "the linear solve
dominates" reading is not by itself a reason to move it — the Mittelmann
NLP suite is factorization-dominated on 36 of its 47 instances, and
`auto_race` still came out slower than the default on 27 of the suite's
42 comparable instances (gh#768).

When a hard problem really is linear-solver-bound, the ordering is worth
a sweep — but sweep the **concrete** variants (`amd`, `amf`, `metis`,
`scotch`, `kahip`) under a capped `max_iter` and pin the winner. Do not
reach for `auto_race` as the cheap way to run that sweep: it optimizes
fill, and fill is a proxy for wall clock that fails on exactly the
structured problems where the ordering matters most (see its row above).
The wins are also concentrated — `qssp180`, at 96 % factorization share
and a fill ratio of 33.9, gives 3.08× for `amd`, while a merely
factorization-*dominated* instance usually gives nothing — so check the
share and the fill first. The full recipe, with the two numbers to check
and where to read them, is under [FERAL ordering: when the adaptive
dispatcher guesses wrong][feral-ordering-recipe].

[feral-ordering-recipe]: troubleshooting.md#feral-ordering-when-the-adaptive-dispatcher-guesses-wrong
