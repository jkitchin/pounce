...
> **Already modelling in Pyomo?** `pyomo_pounce.sens_covariance` computes the
...
> `sens_covariance()` reports the **observed-information** covariance from the
> exact Hessian; they match for linear models and in the small-residual limit and
...
> want scipy-matching numbers); use `sens_covariance()` to interrogate a Pyomo
