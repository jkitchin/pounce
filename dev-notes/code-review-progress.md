...
**Issue.** Failure paths seed the dual inconsistently (`z = 1.0` vs `0.0`) —
`ipm.rs:1116-1120`, `hsde.rs:504` vs `ipm.rs:350-357, 411-418`.

**Verification.** Catalogued every `NumericalFailure` constructor in
pounce-convex:
- `z = vec![1.0; m_ineq]`: `ipm.rs` build-factorization failures (254, 442,
  1113, 1685) and the two `failed` helpers (`hsde.rs:486-498`,
  `hsde_nonsym.rs:1273-...`).
- `z = vec![0.0; m_ineq]`: the cone-cover / nonsym+psd validation guards
  (`ipm.rs:350, 366, 412, 425, 866`).

So the same crate reports two different failure duals. Worse, `z = 1.0` (the
all-ones vector) is the *orthant* identity but is **not a member of a general
dual cone**: an SOC of dimension `m` requires `z₀ ≥ ‖z₁:ₘ‖₂`, i.e.
`1 ≥ √(m−1)`, which fails for `m ≥ 3`. The successful cold start does *not*
hardcode ones — it seeds `z = cone.identity(e)` (`ipm.rs:1196-1201`: orthant →
all-ones, SOC → `(1,0,…,0)`), the genuinely cone-valid centered point.

**Fix.** A `NumericalFailure` solution carries no usable iterate (its `x` and
`y` are already the trivial zero vectors), so the dual should be the simplest
universally-valid value. Unified every failure path on `z = vec![0.0; m_ineq]`
— the cone apex `0`, which lies in *every* closed convex (dual) cone and pairs
naturally with the zero `x`/`y`. Added explanatory comments at
`ipm::failed_solution` and both `failed` helpers cross-referencing each other.
No change to the success path or to the cold-start identity seeding.

**Test.** `tests/batch.rs::pattern_mismatch_failure_seeds_zero_dual` adds an
`ineq_qp` helper (P=2I, one `G` row `Σx ≤ 10`, no bounds ⇒ `m_ineq = 1` with a
non-empty `z`), builds a `QpFactorization` on the n=2 instance, then solves a
structurally-different n=3 instance. That trips the pattern-mismatch guard in
`QpFactorization::solve_inner` (`ipm.rs:1685`). Asserts the result is
`NumericalFailure` with `x = [0;3]`, `y = []`, `z.len() == m_ineq`, and every
`z` entry exactly `0.0`.

**Fail-first.** Reverting just the `solve_inner` failure site to
`vec![1.0; prob.m_ineq()]` makes the test fail at the all-zeros assertion with
...
**Issue.** The float64 decoders take a fast path when the Python value
downcasts to `PyArray1<f64>`, then call `arr.as_slice()` to borrow the buffer.
`as_slice()` requires C-contiguity and returns `Err` otherwise. A *valid but
non-contiguous* float64 array — a strided view such as `x[::2]`, or a
non-owning column of a 2-D array — still downcasts to `PyArray1<f64>`, so it
reaches this path and the `?` turns the contiguity error into a user-visible
`TypeError: The given array is not contiguous`. The generic per-element
fallback that follows is unreachable for such inputs (they downcast fine; only
non-array sequences fall through).

**Affected sites.** `crates/pounce-py/src/problem.rs::extract_f64_vec` (used for
`x0`, bounds, scaling, warm-start duals) and
`crates/pounce-py/src/tnlp_bridge.rs::copy_pyarray_into` (used to copy the
gradient / constraints / Jacobian / Hessian arrays returned by user callbacks).
`extract_index_vec_inferred` (the i64/i32 structure-index decoder) had the same
shape and was fixed for consistency. `warm_start.rs::extract_f64_vec` was
checked and is **not** affected — it decodes via `bound.extract::<Vec<f64>>()`,
which uses the Python iteration protocol and already copies non-contiguous
arrays element-by-element.

**Fix.** Replace `arr.as_slice()?` with an explicit match: on `Ok` keep the
zero-copy slice path; on `Err` (non-contiguous) copy through a strided ndarray
view — `arr.readonly().as_array().iter().copied().collect()` for the value
vectors, and a zip-copy into the output slice for `copy_pyarray_into`. ndarray's
view iterator handles arbitrary strides, so the result is correct for any
layout while the common contiguous case still avoids the copy.

**Verification.** A standalone HS071 reproduction passes bounds, `x0`, and the
gradient/constraints/Jacobian callback returns as non-contiguous strided views.
- **Fail-first**: against the installed (main, pre-fix) build it raises
  `TypeError: The given array is not contiguous`.
- **Fixed**: a release wheel built from the codereview worktree
  (`maturin build` into an isolated venv) solves it to `Solve_Succeeded`,
  obj `17.01402` — the known HS071 optimum.

**Test.** `test_problem.py::test_noncontiguous_float64_arrays_are_copied_not_rejected`
encodes exactly this (a `NonContigHS071` subclass returning strided views, with
non-contiguous bounds/`x0`); it passes against the codereview wheel, and the
...
**Issue.** `scripts/check-docs-consistency.sh` runs `set -euo pipefail`,
then a `python3 - <<'PY' … PY` heredoc that `sys.exit(rc)`s `1` when it
finds an orphan page or a dead TOC link. Immediately after came:
...
Under `set -e`, a non-zero exit from the `python3 …` command aborts the
script *at that command* — so `rc=$?` and the entire friendlier-message
block were unreachable on the one path they existed for (a failing check).
On failure the user saw only python's raw `ORPHAN pages` / `DEAD links`
dump, never the actionable "wire new pages into SUMMARY.md" hint.

**Fail-first (run).** Dropping a throwaway `docs/src/__orphan.md` and
running the script: exit code `1`, but `grep "wire new pages"` on the
output finds nothing — the message block is dead.

**Fix.** Make the checker the condition of an `if`, which `set -e` does not
treat as an abort-worthy failure:
...
(My first attempt used `if ! python3 …` and put `OK` in the `then` branch,
which inverted the logic — the clean tree reported FAILED. Re-running both
paths caught it; dropping the `!` so success→`then`/OK, failure→`else`/exit
1 is correct and clearer.)

**Verification.**
- Failure path (orphan injected): exits `1` **and** prints
  "check-docs-consistency: FAILED — wire new pages into docs/src/SUMMARY.md."
