...
Work branch `claude/pounce-issue-611-loop`, based on `origin/main` at
**`eeca47f922372834a7e8d925ce7fe7b6160c8c7b`**. Every number below was
taken on that SHA with a `maturin develop --release` build, on a 4-core
Linux 6.18.5 container with 15 GB RAM.

**No Rust changed.** `git diff --stat origin/main..HEAD -- '*.rs' Cargo.toml Cargo.lock`
...
Eight commits, all referencing gh #611. `git diff --stat origin/main..HEAD`:
21 files, +7587 / −31 — of which **+3124 / −31 across 19 files is
hand-written**; the remaining +4463 in 2 files is the generated
composite report and its JSON.

```
51a6881d warmstart: the generated composite report
73eec883 warmstart: resistive_network was infeasible at V >= 120
961487fc warmstart: warn when a narrowed --arms drops the correctness reference
ad07ac24 warmstart: composite report, profiles, and docs wired up
6fa8c2c1 warmstart: changed-structure arms — horizon shift and mesh prolongation
f516f458 warmstart: Ipopt through cyipopt, and one KKT residual for both
db6c2571 warmstart: the five initialization arms gh #611 was missing
c048d354 warmstart: families outside the ones warm-start work chose
```
...
| check | result |
|---|---|
| `python -m warmstart.selftest` | 27 families, all pass |
| `pytest tests/test_warm_start*.py tests/test_starts*.py tests/test_sqp_warm_start.py` | **103 passed** |
| `cargo fmt --all` | no diff |
| `git diff origin/main..HEAD -- '*.rs' Cargo.toml Cargo.lock` | empty |
...
moved from `eeca47f9`, so no merge was required.
