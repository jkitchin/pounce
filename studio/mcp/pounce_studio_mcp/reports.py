...
`json.loads` per call, which is negligible.
"""
...
def summarize(report: _native.Report) -> dict[str, Any]:
    return json.loads(report.summarize_json())


def convergence_trace(report: _native.Report) -> dict[str, list]:
    return json.loads(report.convergence_trace_json())


def get_iterate(report: _native.Report, k: int) -> dict[str, Any]:
    try:
        return json.loads(report.get_iterate_json(k))
    except ValueError as e:
        raise ReportError(str(e)) from e
