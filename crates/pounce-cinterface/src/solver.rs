...
unsafe fn kkt_solve_impl(
    ...
    // Guard the back-solve: it runs the linear-solver kernel against the
    // retained factor, which could panic on an unexpected state. A panic
    // unwinding across the `extern "C"` callers (`IpoptSolverKktSolve` /
    // `IpoptSolverKktSolveScaled`) aborts the embedding process; report
    // `FALSE` instead. (See `ffi_guard`.)
...
pub unsafe extern "C" fn IpoptSolverParametricStep(
    ...
    // Guard the sensitivity solve: it runs the linear-solver kernel against
    // the retained factor, which could panic on an unexpected state. A panic
    // unwinding across `extern "C"` aborts the embedding process; report
    // `FALSE` instead. (See `ffi_guard`.)
    crate::ffi_guard(FALSE, || unsafe {
...
pub unsafe extern "C" fn IpoptSolverReducedHessian(
    ...
    // Guard the reduced-Hessian assembly: it runs repeated back-solves against
    // the retained factor, which could panic on an unexpected state. A panic
    // unwinding across `extern "C"` aborts the embedding process; report
    // `FALSE` instead. (See `ffi_guard`.)
    crate::ffi_guard(FALSE, || unsafe {
...
mod tests {
    ...
    /// F5 (session arm): `IpoptSolverSolve` is now wrapped in `ffi_guard`, so
    /// a pounce-internal panic is converted to `Internal_Error` instead of
    /// aborting the embedding process. The secondary half of F5 is the state
    /// hygiene that wrapping demands: the call must invalidate the retained
    /// session factor (`session`) and stats (`problem.last_solve`) **up
    /// front**, so a solve that bails — or whose panic `ffi_guard` catches —
    /// does not leave the handle holding the *previous* solve's converged
    /// factorization (against which a later `IpoptSolverKktSolve` would
    /// silently back-solve) or stale stats.
    ///
    ...
    /// boundary, before unwinding reaches `ffi_guard`; see that fn's note).
    /// So we drive the equivalent control-flow shape: after a successful
    /// solve we corrupt the cached constraint count to a negative value, so
    /// the next `IpoptSolverSolve` returns `InvalidProblemDefinition` from
    /// inside the guarded body **without** reaching the trailing
    /// `session = Some(..)` / `last_solve = Some(..)` writes — exactly where a
    /// caught panic also bails. The up-front clear is what makes the
    /// post-failure state "no data" in both cases.
    #[test]
    fn stale_session_state_cleared_when_resolve_bails() {
        ...
        // A converged solve holds a factor and records stats.
        ...
        // Corrupt the cached constraint count so the next solve bails early in
        // the guarded body (the InvalidProblemDefinition guard) — the same
        // place a caught panic would land — without recording anything.
        unsafe { (*solver).m = -1 };
        let mut x2 = [0.0_f64];
        ...
        // Post-fix: the up-front invalidation dropped the stale factor and
        // stats. Pre-fix both survived — a subsequent KKT back-solve would run
        // silently against the abandoned factorization.
        {
            let info = unsafe { &*solver };
            assert!(
