...
// ===========================================================================
// Branch-and-bound (tree) debugging
// ===========================================================================
//
// Spatial branch-and-bound is a *tree search*, not an iteration loop, so the
// [`DebugState`]/[`DebugHook`] surface above (μ, residuals, an iterate) does
// not describe it. The tree debugger has its own checkpoints and state: a
// node's variable box, the global lower/upper bounds and gap, the frontier
// size, prune reasons, and branching choices. A node's *relaxation* sub-solve
// is itself an interior-point solve, so a tree debugger can still hand a
// [`DebugHook`] down to step into a node — the two surfaces compose.

/// Where in a branch-and-bound search a [`TreeDebugHook`] fired.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum TreeCheckpoint {
    /// A node was popped from the frontier, before it is processed. Its box
    /// and inherited bound are in place; its own relaxation is not yet solved.
    NodeSelected,
    /// The node's relaxation was solved — its lower bound and relaxation
    /// point are available.
    RelaxationSolved,
    /// A new feasible point improved the incumbent (upper bound).
    IncumbentFound,
    /// A node was discarded; see [`TreeDebugState::prune_reason`].
    NodePruned,
    /// The node was split and its children pushed onto the frontier; see
    /// [`TreeDebugState::branch_var`].
    Branched,
    /// The search finished, carrying the outcome via
    /// [`TreeDebugState::status`]. The returned [`DebugAction`] is ignored.
    Terminated,
}

impl TreeCheckpoint {
    /// Stable wire/CLI name (for `stop-at` / the JSON protocol).
    pub fn as_str(self) -> &'static str {
        match self {
            TreeCheckpoint::NodeSelected => "node_selected",
            TreeCheckpoint::RelaxationSolved => "relaxation_solved",
            TreeCheckpoint::IncumbentFound => "incumbent_found",
            TreeCheckpoint::NodePruned => "node_pruned",
            TreeCheckpoint::Branched => "branched",
            TreeCheckpoint::Terminated => "terminated",
        }
    }
}

/// Why a branch-and-bound node was discarded.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PruneReason {
    /// The node's box is infeasible (FBBT or the relaxation proved it).
    Infeasible,
    /// The node's lower bound cannot beat the incumbent (bound dominance).
    BoundDominated,
    /// The box is below the spatial tolerance — a leaf; nothing left to split.
    Leaf,
    /// The node budget was exhausted before this node could be processed.
    NodeLimit,
}
...
impl PruneReason {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            PruneReason::BoundDominated => "bound_dominated",
            PruneReason::Leaf => "leaf",
            PruneReason::NodeLimit => "node_limit",
        }
    }
}

/// Live view of branch-and-bound search state at a [`TreeCheckpoint`].
///
/// Required methods are the quantities every spatial B&B has; the rest are
/// checkpoint-specific (branch variable, prune reason, terminal status) and
/// default to `None`.
pub trait TreeDebugState {
    /// Which checkpoint we are paused at.
    fn checkpoint(&self) -> TreeCheckpoint;

    /// Sequential id of the current node (assignment order).
    fn node_id(&self) -> u64;

    /// Depth of the current node in the tree (root = 0).
    fn depth(&self) -> usize;

    /// Number of nodes processed so far.
    fn nodes_processed(&self) -> u64;

    /// Number of open nodes on the frontier.
    fn frontier_len(&self) -> usize;

    /// The current node's variable box `(lo, hi)`.
    fn node_box(&self) -> (Vec<Number>, Vec<Number>);

    /// The current node's lower bound (its relaxation value). `NaN` before
    /// the relaxation is solved (e.g. at [`TreeCheckpoint::NodeSelected`]).
    fn node_lb(&self) -> Number;

    /// The global lower bound — the minimum bound over all open nodes.
    fn global_lb(&self) -> Number;

    /// The incumbent objective (global upper bound), or `None` if no feasible
    /// point has been found yet.
    fn incumbent(&self) -> Option<Number>;

    /// The optimality gap `upper − lower`; `+∞` when there is no incumbent.
    fn gap(&self) -> Number {
        match self.incumbent() {
            Some(ub) => ub - self.global_lb(),
            None => Number::INFINITY,
        }
    }

    /// The branching variable chosen at [`TreeCheckpoint::Branched`].
    fn branch_var(&self) -> Option<usize> {
        None
    }

    /// Why the node was pruned, at [`TreeCheckpoint::NodePruned`].
    fn prune_reason(&self) -> Option<PruneReason> {
        None
    }

    /// The incumbent point, when one exists (for `print incumbent`).
    fn incumbent_point(&self) -> Option<Vec<Number>> {
        None
    }

    /// Solve outcome, present only at [`TreeCheckpoint::Terminated`].
    fn status(&self) -> Option<&str> {
        None
    }
...
/// [`TreeCheckpoint`]. The CLI's tree REPL is the production implementation.
pub trait TreeDebugHook {
    /// Called at every tree checkpoint. Inspect via `state`, then return
    /// whether to keep searching.
    fn at_node(&mut self, state: &mut dyn TreeDebugState) -> DebugAction;
}
