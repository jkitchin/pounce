...
fn explicit_bool(options: &OptionsList, name: &str) -> Option<bool> {
    match options.get_bool_value(name, "") {
        Ok((v, true)) => Some(v),
        _ => None,
    }
}

fn explicit_number(options: &OptionsList, name: &str) -> Option<Number> {
    match options.get_numeric_value(name, "") {
        Ok((v, true)) => Some(v),
        _ => None,
    }
}
...
impl SensOptionOverrides {
    ...
    pub fn from_options_list(options: &OptionsList) -> Self {
        Self {
            run_sens: explicit_bool(options, "run_sens"),
            compute_red_hessian: explicit_bool(options, "compute_red_hessian"),
            rh_eigendecomp: explicit_bool(options, "rh_eigendecomp"),
            sens_boundcheck: explicit_bool(options, "sens_boundcheck"),
            sens_bound_eps: explicit_number(options, "sens_bound_eps"),
            sens_max_pdpert: explicit_number(options, "sens_max_pdpert"),
        }
    }
