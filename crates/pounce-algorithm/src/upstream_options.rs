...
pub fn register_all_upstream_options(r: &RegisteredOptions) -> Result<(), SolverException> {
    ...
    r.add_lower_bounded_integer_option("dual_diverging_streak", "Consecutive growing-dual-infeasibility iterations before the dual-divergence guard fires (0 disables).", 0, 15, "pounce addition (pounce#246). The primal `diverging_iterates_tol` guard only catches |x| blowing up; a bad warm start can instead send the *dual* infeasibility diverging (inf_du -> 1e14, the inertia regularization -> 1e14, the barrier parameter frozen, full steps still accepted) while |x| stays bounded, so the solve grinds in ever-more-ill-conditioned KKT factorizations that each take seconds. When the dual infeasibility grows for this many consecutive iterations (in the elevated regime) and is large in absolute terms, the outer routes to the restoration phase — the same recovery the least-square-multiplier init path reaches on its own — before the factorizations start choking. Set to 0 to disable.")?;
