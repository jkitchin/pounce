...
pub fn register_all_upstream_options(r: &RegisteredOptions) -> Result<(), SolverException> {
    ...
    r.add_bool_option("filter_theta_roundoff_retry", "Whether a line search that fails at an already-feasible iterate is retried against theta's round-off floor before restoration.", true, "pounce addition (gh#945). WHAT IT DOES: when the filter line search runs out of alpha, the driver hands the iterate to the restoration phase, whose job is to reduce the constraint violation. If that iterate is already feasible to the round-off of its own constraint evaluation, restoration has nothing to minimize: it converges on its first iterate, returns the point it was given, and the solve dies from the optimum with Error_In_Step_Computation. This option makes the driver try one more alpha loop first, with the filter theta axis measured against theta own evaluation noise, and only in that situation. If the retry finds nothing either, the original outcome stands and the hand-off proceeds exactly as before -- so it can only be reached on a trajectory that was otherwise about to enter restoration. WHY THE FILTER IS WHAT BLOCKS IT: upstream FilterEntry::Acceptable compares both of the filter coordinates raw, while every other acceptance test in this line search (armijo_holds, is_sufficient_progress, both axes) allows for round-off. The filter is the one test still reading raw values, and the theta axis is where that bites, because theta has a hard floor the barrier objective does not: on a model whose constraints are LINEAR -- or any model once it is feasible to round-off -- every filter entry records a theta of a few ulp, and every later trial is ranked against it on nothing but which way the last constraint sum rounded. Set to no to restore the plain hand-off. Turning it off is also how the gh#884 fixtures hold their base trajectory fixed, since this retry removes the stall their machinery was built against. THE NUMBERS ARE IN THE TESTS, not here, because a help string is not re-measured when the code moves: the defect and the reported model are in pounce-rs/tests/issue_945_eq_constrained_lbfgs_filter_noise.rs; why this is a gated retry rather than a filter setting, with the measured floor boundary on qpec_small, is in pounce-algorithm/tests/issue_884_biactive_dual_divergence.rs (the_gh945_retry_gate_is_what_this_fixture_relies_on); the floor itself, and why it carries no safety factor on eps and takes the max over rows rather than the 1-norm sum, is on IpoptCq::theta_evaluation_noise_floor; the cap that the gate and the filter both read is on line_search::filter::entry_accepts and its unit test. CHANGELOG.md carries the corpus ledger.")?;
    ...
    r.add_lower_bounded_number_option("bound_relax_obj_cap", "Cap each variable's bound relaxation by its objective sensitivity.", 0.0, false, 0.0, "DELIBERATE DEVIATION FROM UPSTREAM IPOPT (pounce#967), OFF BY DEFAULT (0 = disabled). bound_relax_factor widens the declared box by an ABSOLUTE amount, so its effect on the reported objective is unbounded in the objective's gradient: on min C*x + y/C over [0,1]^2 with C=1e12 the default 1e-8 widening lets x settle at -1e-8 and moves the reported objective by 1e4 -- negative, for an objective that is non-negative everywhere on the declared box, and reported as Solve_Succeeded. When positive, coordinate i's widening is additionally capped at bound_relax_obj_cap / |df/dx_i|, evaluated at the same lifted starting point the gradient-based scaling uses, which bounds the widening's contribution to the objective by this value. Coordinates the objective is insensitive to keep the full widening, which is what the log-barrier needs to hold its iterates strictly interior. Row bounds (d_l/d_u) are untouched: their widening is already scale-relative (pounce#385) and a row's objective contribution is not a per-coordinate quantity. Unlike honor_original_bounds this changes the model the solver sees, so it is a trajectory change and must be swept.")?;
    ...
    r.add_bool_option("feral_refine", "Whether FERAL should run iterative refinement on every back-solve.", true, "THE DEFAULT IS CONDITIONAL AND THIS FLAG CANNOT EXPRESS IT (pounce#909). It is yes on the exact-Hessian path -- the default path, and what this registration reports -- and feral_config_from_options forces it to no under hessian_approximation=limited-memory unless you set it explicitly. Read that carve-out before reading anything below: through 0.11.0 this was registered as no while the exact path ran yes, so pounce --print-options and every comment in the tree describing the exact path disagreed with the binary, and a test that set feral_refine=yes to contrast against the default was setting it to the value it already had. What follows is the argument for OFF, which is why the limited-memory path takes it and why the option exists; the argument for leaving the exact path ON is that turning it off there is a genuine two-sided trade, measured on e5f5830b with one binary: the CLI fixture corpus moves 10 legs on the exact leg with NO status flips and a favourable net (issue_508_infeasible_gap_1em4 441 -> 245, square_flowsheet_resto 54 -> 47, deb7 147 -> 146, pooling_rt2stp 109 -> 107, against mu_fallback_point_floor 31 -> 32), the lbfgs leg does not move at all because refinement is already off there, AC-OPF (PGLib) runs 10-18% faster with iteration counts unchanged and the solution identical to ~13 significant figures -- and NARX_CFy pays 400 -> 630 iterations and 208.7 s -> 254.2 s for it, while eigena2 final dual infeasibility goes 3.43e-10 -> 5.20e-09 (both inside tol). Nothing separates those, so the default is scoped rather than flipped; set it explicitly per problem. The rest of this entry is the OFF case. Off is what every direct linear solver Ipopt ships does, and what pounce's own MA57 does. Per Waechter-Biegler section 3.10, refinement belongs on the *unreduced* Newton system, because the condensation Sigma = S^-1 Z destroys information as mu -> 0 and a backend can only refine the condensed system it factorized. PdFullSpaceSolver::compute_residuals is the unreduced one, so pounce already refines the right system in the right place, capped at max_refinement_steps (10) and accepting at residual_ratio_max (1e-10). Upstream disables the backend loop everywhere: MA27 has no such routine; MA57's MA57D is never declared or called (IpMa57TSolverInterface.cpp:785 calls only ma57c); MUMPS sets icntl[9] = 0 under the comment \"no iterative refinement iterations\"; Pardiso Project registers 0; only MKL Pardiso and WSMP opt in. Turning it on nests FERAL's loop inside pounce's: FERAL's convergence target is hard-wired to eps*sqrt(n), machine precision on the condensed system, which on a large ill-conditioned KKT is unreachable, so the inner loop runs to its cap on every back-solve driving a residual nobody consults to a tolerance nobody set. One augmented-system solve then costs up to 10x11 substitution passes plus a matvec per step. It was on by default in every release through 0.10.0, and the reason was real but incomplete: FERAL defaults to ZeroPivotAction::ForceAccept, so its raw solve can leave real residual against the system it factorized -- something Ipopt's architecture assumes a backend does not do -- and with nothing else to catch that, the gh#590 badly-scaled LP (data scale 1e11) exits RestorationFailed with refinement off. But Ipopt's answer to a factorization that cannot deliver is not to refine inside the backend; it is IncreaseQuality (IpPDFullSpaceSolver.cpp:296) -- escalate the factorization and refactor -- and FeralSolverInterface::increase_quality returned a hard-coded false, so that rung was missing and the inner refinement was standing in for it. Wiring increase_quality through to FERAL's own ladder (scaling Identity -> InfNorm, then pivot_threshold^0.75, the analog of MA57's pivtol = min(pivtolmax, pivtol^0.75)) restores it, and gh#590 then solves with this off. Measured on the 126028-dimension laptime KKT under limited-memory, one binary, three runs back to back: 68.9 s with this on against 18.8 s with it off and the rung wired, next to MA57's 10.7 s, with LinearSystemBackSolve going 54.6 s -> 8.2 s. That 68.9 -> 18.8 s figure is a limited-memory one, and limited-memory is the path the carve-out covers; the exact path's numbers are the two-sided ones at the top of this entry. Set no on an exact-Hessian problem where the timing report shows LinearSystemBackSolve dominating, and re-check the answer; see feral_refine_steps to cap the inner loop instead of leaving it, and feral_refine_target to skip it on back-solves that are already accurate enough. Neither Ma57SolverInterface nor TSymLinearSolver has in-backend refinement, so this duplication is FERAL-only. THE ACTIVE-SET SQP ARM IS A THIRD CASE AND KEEPS THIS ON, measured rather than inherited. That arm has no PdFullSpaceSolver, so the argument above -- refine the unreduced system in the host loop, not the condensed one in the backend -- has no host loop to appeal to: pounce_qp::factor::LinearSolver relies on the backend's own refinement (its own words: refinement is implicit in that layer), and turning it off removes refinement from the stack entirely rather than relocating it. That is also why feral_config_from_options_scoped exists: the limited-memory carve-out above keys off hessian_approximation, which is not that arm's Hessian selector (sqp_hessian is), so the SQP path passes RefineCarveOut::None and does not inherit it. Swept explicitly on that arm, one binary, 194 fixture-legs, feral_refine=no against the default: NO fixture loses a solve, the net is 2 iterations saved across the whole corpus, and the exact leg does not move a single iteration count -- so there is no speed case. What it costs is seven moved objectives, i.e. seven answers at a different point, four of them material: autocorr_bern55-06 -2320 -> -2368 on the exact leg (a model CLAUDE.md already names as one where a retry hands back a different local solution), hs13_bigstart 1 -> 0.9942 exact and 1 -> 1.0018 lbfgs, and eigena2 1420195.742 -> 1420193.049. Two verdicts also move sideways between failures (pooling_rt2stp and eigmaxa, both InfeasibleProblemDetected -> MaximumIterationsExceeded). The one real gain is mu_fallback_point_floor on the lbfgs leg, MaximumIterationsExceeded/200 -> SolveSucceeded/8 at an IDENTICAL objective -- the same answer certified in 8 iterations instead of stalling for 200, which means the refinement is what stalls it there. That is a reason to set this option on a model that stalls, which is what the option is for; it is not a reason to move the default and pay four changed answers for it.")?;
    ...
    r.add_string_option(
        ...
        "auto",
        &[
            ("auto", "Adaptive dispatcher: picks a concrete method per matrix from cheap pattern features. Very-large-and-sparse (n > 100000, avg_deg < 5) → AMD; everything else → AMF. It never selects nested dissection, so collocation / optimal-control / PDE-in-time models must set `metis` explicitly to get it. Pounce default."),
            ("auto_race", "Race-based dispatcher: runs symbolic factorization on AMD and MetisND (feral >= 0.18; AMD, MetisND, ScotchND, KahipND before) and keeps the smallest factor_nnz. ~2× a single symbolic pass, paid once per problem because symbolic factorization is cached across numeric refactorizations with the same pattern. Fill is not wall-clock: the race can keep an arm that factors slower than a pinned method."),
            ...
            ("metis", "feral-metis multilevel nested dissection. The method to pin for collocation, optimal-control and PDE-in-time models, whose KKT is a space-by-time mesh sparse in both directions: 2.1-3.3× faster end to end than the default on GasLib-40 transient control (56k-112k variables) and 2× on the laptime collocation benchmark, with feral >= 0.18 (whose separator refinement fix, feral#203, is what makes it pay). Not a general default: 5% slower (geomean) across Maros-Meszaros and 13-22% slower on grid QPs and single-state optimal control."),
