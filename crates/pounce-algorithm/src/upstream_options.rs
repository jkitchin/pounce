...
pub fn register_all_upstream_options(r: &RegisteredOptions) -> Result<(), SolverException> {
    ...
    r.add_string_option(
        ...
        &[
            ...
            ("auto_race", "Race-based dispatcher: runs symbolic factorization on AMD, MetisND, ScotchND, KahipND and keeps the smallest factor_nnz. ~4× a single symbolic pass, paid once per problem because symbolic factorization is cached across numeric refactorizations with the same pattern. NOT a safe default (pounce gh#768): measured across the Mittelmann NLP suite it is slower than auto on 27 of 42 instances and faster on 1, median 0.82x -- the worst of the ten options screened, on a population where LinearSystemFactorization is 44-98% of OverallAlgorithm. Smallest factor_nnz is not the same objective as smallest wall clock: on robot_a MetisND and KahipND carry ~1.9% more nonzeros in L and factor 25-31% faster, so the race keeps AMD and spends 4.06 s of symbolic (against AMD's 0.56 s) to select the loser; on qssp180 it reaches 1.57x where an amd pin reaches 3.08x. Use it as a one-shot diagnostic of how much the four methods disagree about fill, then pin the concrete winner."),
