...
/// The activity status vocabulary both classifiers report in, shared
/// so `classify_activity` and `reduced_activity` cannot drift apart on
/// the spelling of a class.
fn status_str(codes: &[i8]) -> Vec<&'static str> {
    codes
        .iter()
        .map(|&c| match c {
...
impl PySolver {
    ...
    ///
    /// Taking both labels is deliberate. The ambiguous class contains
    /// genuine kinks whose coordinate is coupled to a neighbour —
    /// `classify_activity` normalizes by the Hessian diagonal, not by
    /// the reduced curvature that generates the multiplier
    /// (pounce#763) — so reading `"ambiguous"` as "not a kink" would
    /// drop real weak rows here. To ask which of them are kinks, call
    /// `reduced_activity` on them.
    ...
    ///
    /// A variable's `q` is the Hessian **diagonal**, and at a kink the
    /// multiplier is generated by the curvature *reduced* along that
    /// coordinate, so `"var_ratio"` there is `reduced/diagonal` and a
    /// genuine kink coupled to a neighbour reports `"ambiguous"` — at
    ...
    /// `classify_activity`'s per-variable verdict for the given
    /// 0-based **user-space** variable indices, re-measured against
    /// the curvature **reduced** along each coordinate instead of the
    /// Hessian diagonal — one back-solve against the held factor per
    /// index (pounce#763).
    ///
    /// `classify_activity` divides a variable's `Σ` by `H_ii`. At a
    /// kink the multiplier is generated by the curvature left after
    /// the other free variables re-optimize, and the two agree only
    /// where the coordinate is decoupled, so its ratio is
    /// `reduced/diagonal` and a genuine kink coupled to a neighbour
    /// reports `"ambiguous"` at any tolerance — the ratio does not
    /// move with `μ`, so a tighter solve reports the same thing. On a
    /// collocation model that coupling is the normal case. Ask here
    /// and the same kink reports `"weakly_active"`.
    ///
    ...
    /// kinks = [v for v, st in zip(red["var"], red["status"]) if st == "weakly_active"]
    /// ```
    ///
    ...
    ///   `classify_activity`'s.
    /// - `"var"`: ndarray of the user variable index each entry
    ///   answers about.
    /// - `"status"`: list of str, from the same vocabulary
    ///   `classify_activity` uses.
    /// - `"ratio"`: ndarray of `Σ_i / |q_i|` on the reduced `q`; NaN
    ///   where nothing was classified.
    /// - `"q_reduced"`: ndarray of the reduced curvature itself,
    ///   signed, in **natural (unscaled) units** like `var_sigma`.
    /// - `"q_sign"`: ndarray of its sign, so an indefinite direction
    ///   is visible rather than hidden by the absolute value.
    /// - `"sigma"`: ndarray of `Σ_i`, identical to the same variable's
    ///   `var_sigma` entry.
    ///
    /// Edge cases: a variable fixed by its bounds reports `"fixed"`
    /// with NaN curvature (it has no column in the factor); a variable
    /// with no finite bound reports its curvature but `"unbounded"`
    /// and a NaN ratio; a coordinate an equality determines outright
    /// has no direction left to reduce along, so its curvature is
    ...
    fn reduced_activity<'py>(
        ...
        let s = self.state.as_ref().ok_or_else(|| {
            PyRuntimeError::new_err("reduced_activity: no converged factor (call solve() first)")
        })?;
        let n_full = s.inner.n_full_x().map_err(solver_error_to_py)?;
        let mut vars = Vec::with_capacity(indices.len());
        for &i in &indices {
            // negative indices reach a raw `usize` cast inside, so they
            // are checked here rather than surfacing as a huge index
            if i < 0 || (i as usize) >= n_full {
                return Err(PyValueError::new_err(format!(
                    "reduced_activity: variable index {i} out of range [0, n={n_full})"
                )));
            }
            vars.push(i as usize);
        }
        let red = s
            .inner
            .reduced_activity(&vars)
            .map_err(solver_error_to_py)?;
        let out = PyDict::new_bound(py);
