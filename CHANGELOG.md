...
- **`AMBIGUOUS` is not "probably not a kink", and there is now an accessor
  ...
  Hessian **diagonal** `H_ii`. At a kink the multiplier is not generated by
  the diagonal — it is generated by the curvature *reduced* along that
  coordinate, i.e. what is left once the other free variables re-optimize.
  Eliminating a free partner `y` from `[[h, c], [c, m]]` leaves `h − c²/m`,
  and `Σ = z/s` equals exactly that, so the reported ratio is
  `reduced/diagonal` and equals 1 only where the coordinate is **decoupled**.
  Couple it and a genuine kink falls out of the `[1e-1, 1e1]` band and reads
  `ambiguous` — at any tolerance, because that ratio is `μ`-independent, so
  re-solving tighter reports the same thing. On a collocation model the
  coupling is the normal case, not a corner.

  New `Solver::reduced_activity(&[i, …])` (Rust) /
  `solver.reduced_activity([i, …])` (Python) re-measures the same rule
  against the reduced curvature, `q_i = 1/(K⁻¹)_ii − Σ_i`: one back-solve
  against the held factor per index, so a coupled kink reads
  `weakly_active` whatever it is coupled to. The subtraction removes the
  barrier's own contribution at `i` while keeping it at every *other*
  coordinate, which is what makes the answer the curvature the multiplier is
  generated against — a strongly active neighbour carries `Σ = O(1/μ)` and
  does not re-optimize, an inactive one carries `O(μ)` and re-optimizes
  freely, and the elimination weights each accordingly.

  The default classification is **unchanged**: the reduced normalizer is the
  reciprocal diagonal of an *inverse*, with no diagonal-of-the-inverse
  shortcut, so classifying every bounded variable that way costs `n`
  back-solves — at 62k variables that is no longer a post-solve diagnostic.
  The refinement is on demand, over the entries in question, and
  `weakly_active_bounds()` already counts `ambiguous` as weak, so nothing in
  the step path changes either. What was wrong was the public *answer*, and
  it was load-bearing: #756 used the class as a proxy for kink-ness and
  shipped a first-order wrong derivative. `classify_activity`'s docs,
  `var_status`, the `AMBIGUOUS` constant, `weakly_active_bounds` and
  `docs/src/sensitivity.md` now all say so.

  Tests: `tests/reduced_activity.rs` covers the four regimes one bounded
  coordinate can be in on a decoupled model (where the refinement must
  *agree* — including the `Σ = 6e9` strongly-active row, which is the
  cancellation check), and
  `the_reduced_normalizer_certifies_a_coupled_kink_at_every_coupling`
  reproduces the issue's own table: four solves that are the same kink, with
